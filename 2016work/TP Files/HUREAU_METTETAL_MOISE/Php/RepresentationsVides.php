<?php
	...
	$requete = ("
		with r1 as (
		select to_char(dateRep, 'DD-MON-YY HH24') dateRep, NoSpec, sum(noPlace) as nbPlacesResa from theatre.LesTickets
		group by dateRep, NoSpec )
		select to_char(dateRep, 'DD-MON-YY HH24') dateRep, NoSpec from theatre.LesRepresentations
		minus
		select dateRep, NoSpec from r1
		order by dateRep
		
		with r1 as (
		select dateRep, Nospec, sum(noPLace) as nbPlaceResa from theatre.lesTickets
		group by dateRep, Nospec )
		select dateRep, noSpec from r1
		where nbPlaceResa >= 1;
		order by dateRep
