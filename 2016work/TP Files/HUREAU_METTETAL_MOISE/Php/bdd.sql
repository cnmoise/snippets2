set linesize 500;
--alter session set current_schema=mettetat;
-- describe
...
create table LesRepresentations(noSpec number(4), dateRep date,
	...
	--constraint p_fk2 foreign key (dateRep) references LesProgrammations(dateDebut)
	);

create table LesProgrammations( noSpec number(4), dateDebut date, dateFin date, type varchar(30),
...
create table LesTickets(noSerie number(4) constraint t_pk1 primary key, noSpec number(4), dateRep date, noPlace number(4), noRang number(4), dateEmission date not null, noDossier number(4),
	...
	constraint t_fk4 foreign key (dateRep,NOSPEC) references LesRepresentations(dateRep, NOSPEC),
...
insert into LesRepresentations select * from theatre.LesRepresentations;
